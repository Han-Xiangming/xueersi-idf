...
/* Built-in test track compiled into the firmware via EMBED_FILES.
 * The symbols below are generated by the build system from "1ezyZip.mp3". */
#define EMBEDDED_PREFIX "(ROM)"
extern const uint8_t _binary_1ezyZip_mp3_start[];
extern const uint8_t _binary_1ezyZip_mp3_end[];
...
static bool open_track(void)
{
    if (!s_src.embedded) {
        ...
        s_src.buf = _binary_1ezyZip_mp3_start;
        s_src.buf_size = (size_t)(_binary_1ezyZip_mp3_end - _binary_1ezyZip_mp3_start);
        s_src.pos = 0;
...
int player_scan(char (*names)[MP3_NAME_LEN], int max, int *count)
{
    *count = 0;
    ...
    if (*count < max) {
        strncpy(names[*count], EMBEDDED_PREFIX " 1ezyZip.mp3", MP3_NAME_LEN - 1);
        names[*count][MP3_NAME_LEN - 1] = '\0';
        (*count)++;
    }
    return 0;
