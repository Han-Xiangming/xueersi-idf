...
分隔线：  y = 20     1px 灰线，整屏宽
主菜单：  y = 22/42/62/82（4 项，20px 行距），光标 ">" 于 x=8，文本 x=18
         底部：左 "[n/4]"（x=4,y=110），右 "A:OK B:BK"（x=104,y=110）
列表页：  4 行 y = 22/40/58/76（18px 行距），光标 x=6，文本 x=16
         滚动窗口：选中项尽量居中（top = sel-1 起 4 行）
设置页：  3 行 y = 30/50/70，值列 x=95；选中项整行青色
...
```text
y=2    书名（去扩展名，96px 内截断，橙）  右上 "12" 或 "12/345"
y=20   分隔线
y=22..109  正文 5 行（(8,22) 152×87，line_space=-7 → 16px 行距；87 = 4×16+23，完整容纳末行字形盒）
y=111  左端 15 格文本进度条（x=8 起）+ 右端百分比（右对齐 x=152）← toast 临时整体替换
```

- 正文单 Label，`LV_LABEL_LONG_MODE_CLIP`；分页由引擎确定性完成（行宽按字体真实度量，全角标点行中/行尾一律半宽压缩），见 `docs/ebook.md`。
- 进度指示器：左端 15 格（x=8 起，~110px，`=` 已读、`-` 未读，四舍五入到格），右端百分比右对齐（x=120 起 32px 宽）；toast 期间两者隐藏，仅居中 toast 文本。
- 总页数后台任务未算完时右上只显当前页。
