...
设置页：  5 行 y = 34/66/98/130/162，值列右对齐至 x=312；选中项整行青色
...
```text
y=34..64   "音量"   值列（右侧右对齐 x=232..312） "80%"
y=66..96   "背光"   "70%"
y=98..128  "蓝牙"   "关 / 开 / 已连接"
y=130..160 "息屏"   "永不 / 15秒 / 30秒 / 60秒 / 120秒 / 300秒"
y=162..192 "重置NVS"   "按A重置"
y=204      "上/下选 A进入 左/右设 B返回"
```

- 文本风精修：行距 30→32px 加宽；值列改为固定右缘（x=232 起 80px 宽，右对齐到 x=312），不同长度值尾端对齐；选中项整行青色不变。
...
y=34   分隔线
y=36..186  正文 5 行（(8,36) 304×150，30px 行高；150 = 5×30，完整容纳 5 行）
