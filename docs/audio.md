...
```text
800Hz 保护高通 → 响度低音架（800Hz，boost 0→+9dB 随音量下降）
  → 分轨 ReplayGain（±12dB，~5ms 平滑）→ 总增益（±12dB，~5ms 平滑）
  → 主音量（~5ms 平滑）→ 软限幅 → int16 钳位（一次性）
```
...
- **链路余量**：完整的增益链为 `保护高通 → 响度低音架 → 分轨 ReplayGain → 总增益 → 主音量 → 软限幅`。前两级（整形）仍按 16 位钳位，其后（从 ReplayGain 起）全程保留 32 位余量，只在写入 I2S DMA / 蓝牙环形缓冲前**一次性**钳到 int16（显式钳位而非截断转换）。早期实现在每个增益级之后都钳到 ±32767，正增益无处可去——灌注被先硬削波、再被后面的音量旋钮衰减，播放中调高「总增益」只会失真而不会变响；保留余量后限幅器才是真正兜住峰值的那一环。
